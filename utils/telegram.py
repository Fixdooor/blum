...
class Accounts:
    ...
    def pars_sessions(self):
        ...
        logger.info(f"Найденные аккаунты: {len(sessions)}!")
        return sessions

    async def check_valid_sessions(self, sessions: list):
        logger.info(f"Проверяю аккаунты...")
        ...
        logger.success(f"Рабочие аккаунты: {len(valid_sessions)}; Нерабочие: {len(sessions)-len(valid_sessions)}")
    ...
    async def get_accounts(self):
        ...
        if not accounts:
            raise ValueError("Нету рабочих аккаунтов!")
        else:
            return accounts
