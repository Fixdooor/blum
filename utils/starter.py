    ...
    while True:
        try:
            ...
            if isinstance(msg, bool) and msg:
                logger.success(f"Thread {thread} | Получил награду!")
            ...
            for task in await blum.get_tasks():
                ...
                if await blum.claim_task(task):
                    logger.success(f"Thread {thread} | Сделал задание «{task['title']}»")
                else:
                    logger.error(f"Thread {thread} | Не получилось сделать задание «{task['title']}»")

            timestamp, start_time, end_time, play_passes = await blum.balance()

            if start_time is None and end_time is None:
                await blum.start()
                logger.info(f"Thread {thread} | Начал фармить!")

            elif start_time is not None and end_time is not None and timestamp >= end_time:
                await blum.refresh()
                timestamp, balance = await blum.claim()
                logger.success(f"Thread {thread} | Получил награду! Баланс: {balance}")
