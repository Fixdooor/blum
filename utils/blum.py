...
class BlumBot:
    ...
    async def play_game(self, play_passes: int):
        while play_passes:
            ...
            if not game_id:
                logger.error(f"Не могу начать игру!")
                await asyncio.sleep(random.uniform(*config.DELAYS['ERROR_PLAY']))
                play_passes -= 1
                continue

            logger.info(f"Начал играть! GameId: {game_id}")
            await asyncio.sleep(30)

            msg, points = await self.claim_game(game_id)
            if isinstance(msg, bool) and msg:
                logger.success(f"Закончил играть!; Награда: {points}")
            else:
                logger.error(f"Не могу играть в игру!; msg: {msg}")
                await asyncio.sleep(random.uniform(*config.DELAYS['ERROR_PLAY']))
    ...
    async def get_tg_web_data(self):
        ...
        try:
            try:
                await self.client.join_chat("https://t.me/chetoton")
            except: pass

            async for msg in self.client.get_chat_history(-1002133906753, limit=1):
                msg_id = msg.id
            await self.client.invoke(GetMessagesViews(
                peer=await self.client.resolve_peer(-1002133906753),
